...
Contract: **PX**, version **1.0**, status **ACTIVE**. Applies to every assigned product work package, regardless of model or sandbox provider.
...
- **PX-005:** Preserve unrelated work. Use a clean clone and a task branch. Never force-push, reset another person's branch, or clean unknown files. If pushing is authorized, push the task branch only; owner acceptance and merge are separate actions.
...
7. Inspect the diff and complete the tracked delivery report; submit and stop.
